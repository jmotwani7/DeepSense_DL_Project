...
destination_root = "../datasets/cityscapes/data/val/label/"
if not os.path.exists(destination_root):
    os.makedirs(destination_root)


root_dir = "../datasets/cityscapes/original_data"
file = glob.glob("../datasets/cityscapes/original_data/val/label/**")
for i in file:
    file_name = os.path.split(i)[-1]
    data = np.load(i)
    # data = np.transpose(data,(2,0,1))
    img = Image.fromarray(np.uint8(data))
    img.save(destination_root + file_name.replace("npy","png"))
